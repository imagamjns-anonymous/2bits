// Aurora Background - uses dynamic import to avoid Vercel ESM compilation
const OGL_CDN = 'https://esm.sh/ogl';
...
class AuroraBg extends HTMLElement {
  constructor() {
    ...
    this.shadowRoot.innerHTML = `
      ...
      <div id="ctn" style="width: 100%; height: 100%;"></div>
    `;
    this.ctn = this.shadowRoot.getElementById('ctn');
    this.resize = this.resize.bind(this);
    this.update = this.update.bind(this);
  }

  async connectedCallback() {
    // Indirect dynamic import — prevents Vercel static analysis from detecting
    // this file as ESM and compiling it to broken CommonJS
    const _import = new Function('url', 'return import(url)');
    const { Renderer, Program, Mesh, Color, Triangle } = await _import(OGL_CDN);

    this.colorStops = [
      this.getAttribute('color-1') || '#7cff67',
      this.getAttribute('color-2') || '#B497CF',
      this.getAttribute('color-3') || '#5227FF'
    ];
    this.amplitude = parseFloat(this.getAttribute('amplitude')) || 1.0;
    this.blend = parseFloat(this.getAttribute('blend')) || 0.5;
